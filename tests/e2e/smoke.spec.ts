...
test('focused media dialog is labelled, dismissible, and restores focus', async ({ page }) => {
  await page.goto('/projects/sepia-client');

  const trigger = page.getByRole('button', { name: 'Focus image: 02 / Reconfigure' });
  await trigger.click();

  const dialog = page.getByRole('dialog', { name: '02 / Reconfigure' });
  await expect(dialog).toBeVisible();
  await expect(dialog).toHaveAccessibleDescription(/same surface can hold changing arrangements/i);

  await page.keyboard.press('Escape');
  await expect(dialog).toBeHidden();
  await expect(trigger).toBeFocused();
});
test('focused media dialog follows image proportions without viewport overflow', async ({ page }) => {
  await page.setViewportSize({ width: 375, height: 812 });
  await page.goto('/projects/sepia-client');

  const mobileTrigger = page.getByRole('button', { name: 'Focus image: 06 / Focus' });
  ...
  const mobileMetrics = await mobileDialog.evaluate((dialog) => {
    ...
    const imageScroll = dialog.querySelector('.sepia-media-dialog__image-scroll');
  ...
  expect(mobileMetrics.dialogWidth - mobileMetrics.dialogLeft).toBeGreaterThanOrEqual(9);
  expect(mobileMetrics.imageHeight / mobileMetrics.imageWidth).toBeCloseTo(1600 / 788, 1);
  expect(mobileMetrics.scrollWidth).toBeLessThanOrEqual(mobileMetrics.dialogWidth + 1);
  expect(mobileMetrics.imageScrollContentHeight).toBeGreaterThan(mobileMetrics.imageScrollHeight);
  ...
  const desktopTrigger = page.getByRole('button', { name: 'Focus image: 01 / Compose' });
  ...
  expect(Math.abs(desktopMetrics.dialogLeft - (desktopMetrics.viewportWidth - desktopMetrics.dialogWidth) / 2)).toBeLessThan(2);
  expect(desktopMetrics.imageHeight / desktopMetrics.imageWidth).toBeCloseTo(982 / 1600, 1);
  expect(desktopMetrics.scrollWidth).toBeLessThanOrEqual(desktopMetrics.dialogWidth + 1);
  await page.keyboard.press('Escape');
