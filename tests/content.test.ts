...
import { capabilities, capabilityDossiers, getCapability, getProject, getStory, personalProjectDossiers, projects, sepiaDossier, stories, storyDossiers } from '@/lib/content';
import { filterByTag, parseFrontmatter, portfolioItems, published, repository, searchPortfolio } from '@/lib/portfolio-repository';
describe('content model', () => {
  ...
  it('has the generated SEPIA dossier', () => expect(sepiaDossier?.title).toBe('SEPIA'));
  ...
  it('contains required dossier sections', () => {
    for (const dossier of [...capabilityDossiers, ...storyDossiers, ...(sepiaDossier ? [sepiaDossier] : [])]) {
  ...
  it('supports scalable metadata and repository access', () => {
    ...
    expect(repository.listStories().every((item) => item.metadata.project === 'SEPIA')).toBe(true);
    expect(repository.listCapabilities().every((item) => item.metadata.type === 'capability')).toBe(true);
    expect(portfolioItems.length).toBeGreaterThan(10);
  });
  it('supports static search and tag filtering', () => {
    expect(searchPortfolio('recovery').some((item) => item.slug === 'configuration-recovery')).toBe(true);
    expect(filterByTag('flutter').some((item) => item.metadata.project === 'SEPIA')).toBe(true);
  });
  it('parses simple future frontmatter', () => {
    expect(parseFrontmatter('---\ntype: personal-project\nfeatured: false\n---\n# Demo').type).toBe('personal-project');
  });
  it('fails closed for non-public and invalid visibility values', () => {
    expect(parseFrontmatter('---\nvisibility: private\n---\n# Draft').visibility).toBe('private');
    expect(published([{ metadata: { status: 'published', visibility: 'private' }, title: 'Private' } as never])).toEqual([]);
  });
