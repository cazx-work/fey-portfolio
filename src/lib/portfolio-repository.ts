...
export type PortfolioType =
  'professional-project' | 'personal-project' | 'story' | 'capability';
...
export type PortfolioItem = {
  ...
  tags: string[];
  category: string;
  roleFit?: import('./content').ProjectRole[];
  problemDomain?: string;
  systemBoundary?: string;
  evidence?: import('./content').ProjectEvidence;
};

const metadataFor = (
  item: Dossier,
  fallback: PortfolioMetadata,
): PortfolioMetadata => ({
...
const professionalProjectMetadata: PortfolioMetadata = {
...
const storyMetadata: PortfolioMetadata = {
  type: 'story',
  project: 'SEPIA',
  visibility: 'public',
  status: 'published',
...
const capabilityMetadata: PortfolioMetadata = {
  type: 'capability',
  project: 'SEPIA',
  visibility: 'public',
  status: 'published',
...
export const portfolioProjects: PortfolioItem[] = projects.map((item) => ({
  ...
  metadata: metadataFor(
    item.dossier,
    item.slug === 'sepia-client'
      ? professionalProjectMetadata
      : {
          type: 'professional-project',
          project: item.dossier.metadata.project ?? 'Experience Digital',
          visibility: item.dossier.metadata.visibility,
          status: item.dossier.metadata.status,
          featured: item.dossier.metadata.featured,
          tags: item.tags,
        },
  ),
  ...
  problemDomain: item.problemDomain,
  systemBoundary: item.systemBoundary,
  evidence: item.evidence,
}));
export const portfolioPersonalProjects: PortfolioItem[] =
  personalProjectDossiers.map((item) => ({
    slug: item.slug,
    title: item.title,
    summary: item.executive.overview,
    metadata: metadataFor(item, {
      type: 'personal-project',
      project: item.metadata.project ?? 'Personal',
      visibility: 'public',
      status: 'published',
      featured: false,
      tags: [],
...
export const portfolioStories: PortfolioItem[] = storyDossiers.map((item) => ({
  slug: item.slug,
  title: item.title.replace(/^SEPIA\s+/i, ''),
  summary: item.executive.overview,
  metadata: metadataFor(item, storyMetadata),
  dossier: item,
  tags: item.metadata.tags,
  category: 'Engineering story',
}));
export const portfolioCapabilities: PortfolioItem[] = capabilityDossiers.map(
  (item) => ({
    slug: item.slug,
    title: item.title,
    summary: item.executive.overview,
    metadata: metadataFor(item, capabilityMetadata),
    dossier: item,
    tags: item.metadata.tags,
    category: 'Capability',
  }),
...
export function searchPortfolio(query: string, items = portfolioItems) {
  ...
  return published(items).filter((item) =>
    [item.title, item.summary, item.metadata.project, ...item.metadata.tags]
      .join(' ')
      .toLowerCase()
      .includes(normalized),
  );
...
export function relatedContent(item: PortfolioItem, limit = 3) {
  return published(portfolioItems)
    .filter(
      (candidate) =>
        candidate.slug !== item.slug &&
        candidate.metadata.project === item.metadata.project &&
        candidate.metadata.type !== item.metadata.type,
    )
    .slice(0, limit);
}
...
export const repository = {
  ...
  getProject: (slug: string) =>
    publicItem(portfolioProjects.find((item) => item.slug === slug)),
  listPersonalProjects: () => published(portfolioPersonalProjects),
  getPersonalProject: (slug: string) =>
    publicItem(portfolioPersonalProjects.find((item) => item.slug === slug)),
  listStories: () => published(portfolioStories),
  getStory: (slug: string) =>
    publicItem(portfolioStories.find((item) => item.slug === slug)),
  listCapabilities: () => published(portfolioCapabilities),
  getCapability: (slug: string) =>
    publicItem(portfolioCapabilities.find((item) => item.slug === slug)),
  relatedContent,
