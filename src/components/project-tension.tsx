...
export function ProjectTension({ stages }: ProjectTensionProps) {
  ...
  return (
    <div className="project-brief__tension-control">
      <div className="project-brief__stages grid grid-cols-3 gap-2 text-center text-xs text-[var(--muted)]" role="group" aria-label="Select a control boundary">
        {stages.map((stage, index) => {
          ...
          return (
            ...
            >
              ...
              <strong className={`block ${isActive ? 'text-[var(--accent)]' : 'text-[var(--ink)]'}`}>{stage.title}</strong>
