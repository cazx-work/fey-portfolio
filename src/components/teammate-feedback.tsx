...
export function TeammateFeedback({
  testimonials,
  limit,
}: TeammateFeedbackProps) {
  const visibleTestimonials = limit
    ? testimonials.slice(0, limit)
    : testimonials;
  const [selectedTestimonial, setSelectedTestimonial] =
    useState<Testimonial | null>(null);
  const [dialogType, setDialogType] = useState<'testimony' | 'profile' | null>(
    null,
  );
  ...
  useEffect(() => {
    if (!selectedTestimonial || !dialogType) return;

    const dialogRef =
      dialogType === 'testimony' ? testimonyDialogRef : profileDialogRef;
    if (dialogRef.current && !dialogRef.current.open) {
      dialogRef.current.showModal();
    }
  }, [dialogType, selectedTestimonial]);
  ...
  function closeDialog(type: 'testimony' | 'profile') {
    const dialogRef =
      type === 'testimony' ? testimonyDialogRef : profileDialogRef;
    dialogRef.current?.close();
    setSelectedTestimonial(null);
    setDialogType(null);
  }

  function handleDialogClose(type: 'testimony' | 'profile') {
    const dialogRef =
      type === 'testimony' ? testimonyDialogRef : profileDialogRef;
    setSelectedTestimonial(null);
    setDialogType(null);
    ...
    <>
      {visibleTestimonials.length > 0 ? (
        <div className="grid gap-5 md:grid-cols-2">
          {visibleTestimonials.map((testimonial, index) => (
            ...
            >
              <button
                type="button"
                className="testimonial-card__testimony flex-1 text-left text-lg leading-8 text-[var(--ink)]"
                onClick={() => openDialog(testimonial, 'testimony')}
              >
                <span
                  className="mr-2 text-2xl text-[var(--accent)]"
                  aria-hidden="true"
                >
                  “
                </span>
                {testimonial.summary}
                <span
                  className="ml-1 text-2xl text-[var(--accent)]"
                  aria-hidden="true"
                >
                  ”
                </span>
                <span className="testimonial-expand-label">
                  <span aria-hidden="true">＋ </span>Read full testimony
                </span>
              </button>
              <figcaption className="testimonial-card__info mt-6 border-t border-[var(--line)] pt-4">
                <button
                  type="button"
                  className="testimonial-profile-button flex w-full items-center gap-3 text-left"
                  onClick={() => openDialog(testimonial, 'profile')}
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="feedback-wrap font-medium">
                      {testimonial.author}
                    </p>
                    <p className="feedback-wrap text-sm text-[var(--muted)]">
                ...
                {testimonial.linkedin && (
                  <a
                    className="testimonial-link mt-3 inline-block text-sm text-[var(--accent)] underline underline-offset-4"
                    href={testimonial.linkedin}
                    target="_blank"
                    rel="noreferrer"
                  >
                    View LinkedIn profile <InlineIcon name="external" />
                  </a>
                )}
                {testimonial.themes.length > 0 && (
        ...
        <p className="border-y border-[var(--line)] py-6 text-[var(--muted)]">
          No published recommendations are available yet.
        </p>
      )}
      <dialog
        ref={testimonyDialogRef}
        className="testimonial-dialog"
        aria-labelledby="testimonial-dialog-title"
        onCancel={() => closeDialog('testimony')}
        onClose={() => handleDialogClose('testimony')}
        onClick={(event) => {
        ...
        {selectedTestimonial && (
          <article className="testimonial-dialog__content">
            <div className="testimonial-dialog__header">
              <p className="font-mono text-xs uppercase tracking-[.14em] text-[var(--accent)]">
                Full testimony
              </p>
              <button
                className="testimonial-dialog__close"
                type="button"
                onClick={() => closeDialog('testimony')}
              ...
              <blockquote className="border-t border-[var(--line)] pt-6 text-lg leading-8 text-[var(--ink)]">
                {selectedTestimonial.quote
                  ...
                  .map((paragraph, paragraphIndex) => (
                    <p className="testimonial-paragraph" key={paragraphIndex}>
                      {paragraph}
                    </p>
                  ))}
              </blockquote>
            ...
            <footer className="testimonial-dialog__footer">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <h2 id="testimonial-dialog-title" className="font-medium">
                    {selectedTestimonial.author}&apos;s testimony
                  </h2>
                  <p className="feedback-wrap text-sm text-[var(--muted)]">
                    {selectedTestimonial.role} ·{' '}
                    {selectedTestimonial.focusedCompany ??
              ...
              {selectedTestimonial.linkedin && (
                <a
                  className="testimonial-link mt-4 inline-block text-sm text-[var(--accent)] underline underline-offset-4"
              ...
              {selectedTestimonial.themes.length > 0 && (
                <div className="profile-tags">
                  {selectedTestimonial.themes.map((theme) => (
                    <span key={theme}>{theme}</span>
                  ))}
                </div>
              )}
      ...
      <dialog
        ...
        className="testimonial-dialog testimonial-profile-dialog"
        aria-labelledby="profile-dialog-title"
        onCancel={() => closeDialog('profile')}
