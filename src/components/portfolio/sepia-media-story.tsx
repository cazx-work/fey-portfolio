...
const desktopFrames: MediaFrame[] = [
  {
    ...
    label: '01 / Compose',
    detail:
      'A wide control surface makes signal paths and module placement visible together.',
    width: 2646,
    height: 1624,
  },
  {
    ...
    detail:
      'The same surface can hold changing arrangements without hiding the surrounding context.',
    width: 2646,
    height: 1624,
  },
];
...
const tabletFrames: MediaFrame[] = [
  {
    ...
    label: '03 / Extend',
    detail:
      'The tablet view preserves the composition model while giving the operator more room for touch.',
    width: 2098,
    height: 1604,
  },
  {
    ...
    label: '04 / Inspect',
    detail:
      'A second tablet state shows how the surface remains legible as the arrangement changes.',
    width: 2098,
    height: 1604,
  },
  {
    ...
    label: '05 / Control',
    detail:
      'The product keeps the operational surface close to the visual model.',
    width: 2098,
    height: 1604,
  },
];
...
const mobileFrames: MediaFrame[] = [
  {
    ...
    label: '06 / Focus',
    detail:
      'On a phone, the same product is reduced to a focused, vertically navigable surface.',
    width: 800,
    height: 1624,
  },
  {
    ...
    label: '07 / Navigate',
    detail:
      'Navigation stays explicit instead of relying on a desktop-sized canvas.',
    width: 800,
    height: 1624,
  },
  {
    ...
    label: '08 / Adapt',
    detail:
      'Different form factors present the same control story at different densities.',
    width: 800,
    height: 1624,
  },
];
...
export function SepiaMediaStory() {
  ...
  return (
    <>
      <section
        className="sepia-media-story"
        aria-labelledby="media-story-heading"
      >
        <div className="sepia-media-story__intro">
          <div>
            <p className="case-study-label">Selected evidence</p>
            <h2 id="media-story-heading">
              One control model, three working surfaces
            </h2>
          </div>
          <p className="sepia-media-story__lede">
            The supplied recordings and stills show the product at desktop,
            tablet, and phone sizes. Read them as one connected surface: compose
            a signal path, inspect its modules, and carry that state into the
            next context.
          </p>
        </div>
        ...
        <figure className="sepia-media-story__walkthrough">
          <div className="sepia-media-story__video-frame">
            ...
            >
              ...
              Your browser does not support the SEPIA walkthrough video. Use the
              still sequence below instead.
            </MediaVideo>
          </div>
          <figcaption
            id="sepia-video-caption"
            className="sepia-media-story__video-caption"
          >
            <span>
              <strong>Walkthrough</strong> / responsive product recording
            </span>
        ...
        <div className="sepia-media-story__sequence">
          <MediaChapter
            title="The wide view"
            description="Desktop screenshots establish the full composition: paths, modules, library, and the surrounding control context."
            frames={desktopFrames}
            onFrameClick={openFrame}
            featured
          />
          <MediaChapter
            title="The working view"
            description="Tablet screenshots keep the visual model intact while changing the available space and interaction posture."
            frames={tabletFrames}
            onFrameClick={openFrame}
          />
          <MediaChapter
            title="The focused view"
            description="Mobile screenshots show the same product language compressed into a navigable, focused surface."
            frames={mobileFrames}
            onFrameClick={openFrame}
          />
        </div>
        ...
        {focusedFrame && (
          <figure className="sepia-media-dialog__content">
            <div className="sepia-media-dialog__header">
              <p id="sepia-media-dialog-title" className="case-study-label">
                {focusedFrame.label}
              </p>
              <button
                type="button"
                className="sepia-media-dialog__close"
                onClick={closeFrame}
            ...
            <div
              className="sepia-media-dialog__image-scroll"
              aria-label="Scrollable focused image"
            >
              <MediaImage
            ...
            <figcaption id="sepia-media-dialog-description">
              {focusedFrame.detail}
            </figcaption>
          </figure>
        )}
...
function MediaChapter({
  ...
  description: string;
  frames: MediaFrame[];
  onFrameClick: (frame: MediaFrame) => void;
  featured?: boolean;
}) {
  return (
    <section
      className={`sepia-media-chapter ${featured ? 'sepia-media-chapter--featured' : ''}`}
    ...
    >
      <div className="sepia-media-chapter__heading">
        <div>
          ...
          <h3 id={`${title.toLowerCase().replaceAll(' ', '-')}-heading`}>
            {description}
          </h3>
        </div>
        <span className="sepia-media-chapter__count">
      ...
      <div className="sepia-media-chapter__frames">
        {frames.map((frame) => (
          <figure className="sepia-media-frame" key={frame.src}>
            <button
              type="button"
              className="sepia-media-frame__button"
              onClick={() => onFrameClick(frame)}
              aria-label={`Focus image: ${frame.label}`}
            >
              <div className="sepia-media-frame__image">
                <MediaImage
                  ...
                  sizes={
                    frame.width < 1000
                      ? '(max-width: 639px) 45vw, 18rem'
                      : '(max-width: 639px) 100vw, 52rem'
                  }
                />
              </div>
            </button>
