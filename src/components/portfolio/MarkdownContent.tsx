...
function MarkdownBlockView({ block }: { block: MarkdownBlock }) {
  if (block.type === 'heading') {
    return block.level === 2 ? (
      <h2 id={block.id} data-section-level="2">
        {renderInlineMarkdown(block.text)}
      </h2>
    ) : block.level === 3 ? (
      <h3 id={block.id} data-section-level="3">
        {renderInlineMarkdown(block.text)}
      </h3>
    ) : (
      <h4 id={block.id} data-section-level="4">
        {renderInlineMarkdown(block.text)}
      </h4>
    );
  }
  ...
  if (block.type === 'table') {
    return (
      <div
        ...
        role="region"
        aria-label="Technical evidence table"
        tabIndex={0}
      >
...
function renderInlineMarkdown(text: string) {
  ...
  return parts.map((part, index) => {
    ...
    if (link) {
      return (
        <a key={`${part}-${index}`} href={resolveMarkdownLink(link[2])}>
          {link[1]}
        </a>
      );
    }
...
function resolveMarkdownLink(href: string) {
  ...
  if (
    pathname.endsWith(
      '/contents/journals/rive-animation-performance-optimization.md',
    )
  ) {
    return `/engineering-stories/interaction-performance${anchor ? suffix : '#evidence'}`;
  }

  const storyMatch = pathname.match(
    /(?:^|\/)stories-content-dossier\/([^/]+)\.md$/,
  );
  if (storyMatch) {
  ...
  const capabilityMatch = pathname.match(
    /(?:^|\/)capabilities-content-dossier\/([^/]+)\.md$/,
  );
